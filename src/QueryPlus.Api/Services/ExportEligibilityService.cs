...
public sealed class ExportEligibilityService
{
    ...
    public void MarkEligible(string username, int procedureId, IDictionary<string, string?> values, int rowCount)
    {
        ...
        _entries[Key(username)] = new(procedureId, Hash(values), rowCount, DateTime.UtcNow);
    }

    public void Clear(string username)
    {
        if (!string.IsNullOrWhiteSpace(username)) _entries.TryRemove(Key(username), out _);
    }
    ...
    public bool TryValidate(string username, int procedureId, IDictionary<string, string?> values, out string? error)
    {
        ...
        if (string.IsNullOrWhiteSpace(username) || procedureId <= 0 ||
            !_entries.TryGetValue(Key(username), out var entry))
        {
        ...
        if (Hash(values) != entry.ParameterHash)
        {
    ...
    private static string Hash(IDictionary<string, string?> values) => Convert.ToHexString(
        SHA256.HashData(Encoding.UTF8.GetBytes(string.Join("&",
            values.OrderBy(x => x.Key, StringComparer.OrdinalIgnoreCase).Select(x =>
                $"{x.Key.ToLowerInvariant()}={(x.Value ?? string.Empty).Trim()}")))));
