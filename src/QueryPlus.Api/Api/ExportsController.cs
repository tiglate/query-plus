    ...
    ICurrentUserContext user) : ControllerBase
{
    ...
    public async Task<IActionResult> Queue(ExportRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!eligibility.TryValidate(user.Username, request.ProcedureId, values, out var reason))
            return BadRequest(Problem(title: "Export is not eligible", detail: reason, statusCode: 400));
        ...
        var id = exports.QueueExport(request.ProcedureId, values, user.Username);
        return AcceptedAtAction(nameof(Status), new { jobId = id },
            new { jobId = id, status = ExportJobStatus.Queued });
    }
    ...
    public IActionResult Status(Guid jobId)
    {
        ...
        if (job is null || !Owned(job)) return NotFound(Problem(title: "Export job not found", statusCode: 404));
        return Ok(job);
    }
    ...
    public IActionResult Download(Guid jobId)
    {
        ...
        return path is null
            ? NotFound(Problem(title: "Export file is not available", statusCode: 404))
