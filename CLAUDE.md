...
docker compose up -d mailpit                  # SMTP catch-all for Jobs module notifications, UI at :8025
...
### Demo data seeding and production startup safety nets

`DemoDataSeeder` (`src/QueryPlus.Data/Seed/DemoDataSeeder.cs`, invoked from `Program.cs` via `app.SeedDemoDataAsync()`) always applies EF migrations, in every environment. It only installs the demo SQL objects/catalog (`demo-objects.sql`, `demo-catalog.json`) when `Database:SeedDemoDataOnStartup` is `true` — layered per environment via `appsettings.{Environment}.json`: `true` in Development/Docker, `false` in the base `appsettings.json` (so Production and any environment without its own override get no demo data by default). Independently of that flag, `DemoDataSeeder` also checks `INFORMATION_SCHEMA.TABLES` after migrations and refuses to seed demo data if the database already contains any table it didn't create (derived from the live EF model + a regex parse of `demo-objects.sql`, so the known-table set can't drift) — this check cannot be disabled by configuration. `ProductionSecretsValidator` (`src/QueryPlus.Api/Hosting/`) runs at startup and throws when `ASPNETCORE_ENVIRONMENT=Production` and `ConnectionStrings:DefaultConnection`/`Keycloak:ClientSecret` are empty or still the documented local-dev dummy values. `GET /api/health` is liveness-only; `GET /api/health/ready` additionally does a real `CanConnectAsync()` against SQL Server. See `docs/deploy-producao.md` §4.2 for the full production rationale.
...
- `tests/QueryPlus.Infrastructure.Tests` — currently just `LinuxRunAsUserCatalogTests.cs`, covering `LinuxRunAsUserCatalog.ParseEligibleUsers` (the `getent passwd` line-parsing/denylist filtering pulled out as a pure `public static` method specifically so it's unit-testable without mocking `Process`/shelling out to `getent`).
