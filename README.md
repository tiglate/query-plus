...
**Current (Phases 0–3):** `_Layout` loads `~/dist/js/app.js` for shell (nav, confirm, CSRF, SheetGrid) and home page controller (`data-page="home"`). `site.js` still has admin-only helpers. Styles still come from `wwwroot/css/site.css` (plus CDNs). Build ClientApp before first run if `wwwroot/dist` is missing (`pnpm run build` or `dotnet build` auto-builds when dist is absent).
