...
| `GET` | `/api/health` | Verificação de liveness (`{status: healthy}`) — só confirma que o processo está de pé | Anônimo |
| `GET` | `/api/health/ready` | Verificação de readiness — `CanConnectAsync()` real contra o SQL Server; `503 {status: unhealthy}` se inacessível | Anônimo |
